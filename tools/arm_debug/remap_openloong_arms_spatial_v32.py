...
DEFAULT_OUT_DIR = ROOT / "output/openloong_v32_handpose"
...
class OpenLoongArmSpatialRetargeter:
    ...
    def solve(
        ...
        def residual(z: np.ndarray) -> np.ndarray:
            ...
            # V3.2 V6 hand long-axis direction residual.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "OpenLoong V3.2 bilateral 14-DoF arm spatial + V6 finger direction "
            "+ Palm Roll V2 hand orientation retargeting. Body qpos is frozen "
            "and both arms are jointly re-solved from frame 0."
        )
    )
    ...
    p.add_argument(
        ...
        default=0.90,
        help=(
            "V3.2 V6 hand long-axis direction weight inside the 14-DoF IK."
        ),
    )
    p.add_argument(
        "--palm-dir-weight",
        ...
        default=1.25,
        help=(
            "V3.2 Palm Roll V2 palm-normal direction weight inside the 14-DoF IK."
        ),
    )
    p.add_argument(
        "--palm-roll-sign-left",
...
def main() -> None:
    ...
    print(
        "OpenLoong V3.2 "
        "Arm Spatial + Hand Pose Retargeting"
    )
    print("=" * 80)
    print(
        "body CSV :",
