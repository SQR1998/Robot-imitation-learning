...
OUT_DIR = (
    ROOT
    / "output/linglong20_arm_v14_extension_antiflip"
)

OUT_CSV = (
    OUT_DIR
    / "linglong20_arm_v14_extension_antiflip_qpos37.csv"
)
...
def solve_side(
    ...
    print(
        side.upper(),
        "ARM V1.4 EXTENSION + ANTI-FLIP SOLVE",
    )
    print("=" * 110)
    ...
    _straight_ids = np.flatnonzero(
        human_bend_deg_seq <= 8.0
    )
    
    print(
        f"{side} human fully-straight "
        f"(<=8deg) frames:",
        len(_straight_ids),
    )
...
def main():
    ...
    print("=" * 110)
    print(
        "LINGLONG ARM V1.4 EXTENSION + ANTI-FLIP IK"
    )
    print("=" * 110)
